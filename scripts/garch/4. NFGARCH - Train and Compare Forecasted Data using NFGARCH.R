...
for (config_name in names(model_configs)) {
  ...
  for (asset in names(fx_returns)) {
    ...
    if (!key %in% names(nf_residuals_map)) {
      message(paste("❌ Skipped:", key, "- No synthetic residuals found."))
      next
    }
...
for (config_name in names(model_configs)) {
  ...
  for (asset in names(valid_fx_returns)) {
    key <- paste0(config_name, "_", "fx_", asset, "_residuals_synthetic")
    if (!key %in% names(nf_residuals_map)) {
      message(paste("⚠️ Missing NF residuals for", key))
      next
    }
    
    message("🔁 Running NF-GARCH CV (FX): ", asset, " [", config_name, "]")
  ...
  for (asset in names(valid_equity_returns)) {
    ...
    message("🔁 Running NF-GARCH CV (EQ): ", asset, " [", config_name, "]")
